...
*   **选题管理与内容编辑**: 
    *   **选题界面**: 
        *   初始加载 `src/config/topicsMeta.json` 中的选题元信息（ID, 标题, 简介）来构建基础选题列表。
        *   应用加载后，后台调用 API (`/api/list-content-files`)，扫描 `src/markdown/` 目录，获取每个 `.md` 文件的详细信息（标题、描述、卡片数等），并与 `topicsMeta.json` 对比和同步。
...
*   **性能优化**: 
    ...
    *   实现了选题列表的两阶段加载（先快速加载基础信息，再后台同步详细状态），优化用户体验。
